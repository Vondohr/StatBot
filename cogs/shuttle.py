...
WAIT_BEFORE_REVOKE = 1 * 60  # 5 minutes
# WAIT_BEFORE_REVOKE = 5 * 60  # 5 minutes
...
# Store users currently on cooldown
active_shuttles = set()
...
class Shuttle(commands.Cog):
    ...
    def extract_destination(self, role_name: str) -> str:
        for prefix in VALID_PREFIXES:
            if role_name.startswith(prefix + " "):
                return role_name[len(prefix) + 1:]  # Removes prefix and space
        return None
    ...
    async def shuttle(self, interaction: discord.Interaction, location: str):
        ...
        shuttle_duration = random.randint(1, 2)
        # shuttle_duration = random.randint(40, 55)
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"Click **Cancel** within 5 minutes to abort.\n\n"
                f"Flight duration: `{shuttle_duration}` minutes."
            ),
        ...
        # Remove old planet/moon roles
        for role in interaction.user.roles:
            if any(role.name.startswith(prefix) for prefix in VALID_PREFIXES):
                await interaction.user.remove_roles(role)

        # Wait the shuttle travel time
        await asyncio.sleep(shuttle_duration * 60)

        new_location_role = discord.utils.get(interaction.guild.roles, name=location)
        if not new_location_role:
            await interaction.followup.send("Could not find the destination role!", ephemeral=True)
            active_shuttles.discard(user_id)
            return

        try:
            await interaction.user.add_roles(new_location_role)
        ...
        # Find forum channel (partial match)
        destination_lower = destination.lower()
        forum_channel = discord.utils.find(
            lambda c: isinstance(c, discord.ForumChannel) and destination_lower in c.name.lower(),
            interaction.guild.channels
        )

        if not forum_channel:
            await interaction.followup.send(f"Forum channel containing `{destination_lower}` not found.", ephemeral=True)
            active_shuttles.discard(user_id)
            return

        # Find thread with 🚀 in name
        target_thread = None
        ...
        arrival_embed = discord.Embed(
            title=f"{interaction.user.display_name} has landed",
            description=f"{interaction.user.mention} has arrived at **{location}**.",
            color=discord.Color.blue()
        )
        embed.set_image(url="https://64.media.tumblr.com/05fb6dde48bb81815d939ee4f7ed000c/eec4ede0fa31de36-c9/s540x810/0fe51ba6ec2bd34a5fc46b247bf34e6a5af18cb4.gif")
